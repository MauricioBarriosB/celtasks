...
import { Zap, Menu, X, Home, FolderKanban, Shield, User, LogOut, Settings, Mail, BarChart3 } from "lucide-react";
import { useAuthContext } from "@modules/user/context/AuthContext";

const publicNavItems = [{ name: "Home", href: "/home", icon: Home }];

const contactNavItem = { name: "Contact", href: "/contact", icon: Mail };
...
const adminNavItems = [{ name: "Admin", href: "/admin", icon: Shield }];
...
export default function Layout({ children }: Readonly<LayoutProps>) {
    ...
    const navItems = isAuthenticated
        ? [...publicNavItems, ...protectedNavItems, ...(isAdmin ? adminNavItems : []), contactNavItem]
        : [...publicNavItems, contactNavItem];
    ...
    return (
        ...
        >
            ...
            <main className="flex-1 px-4 pt-6 pb-4 lg:px-25 lg:pt-8 lg:pb-8">{children}</main>

            <footer className="border-t border-default-100 py-6 px-8 bg-black">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
                    <p className="text-sm text-default-600">
                        © {new Date().getFullYear()} CelTasks. All rights reserved.
                    </p>
                    <NavLink to="/contact" className="text-sm text-default-500 hover:text-primary transition-colors">
                        Contact & Support
                    </NavLink>
                </div>
            </footer>
