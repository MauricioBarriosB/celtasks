...
import { fetchTask, updateTask, deleteTask, fetchProjectUsers, fetchProject } from "@services/apiCrud";
...
export default function TaskDetail() {
    ...
    function backToProject() {
        if (!task) return;
        const view = localStorage.getItem(`celtasks_kanban_view_${task.projectId}`);
        const target = view === "list" ? "list" : "board";
        navigate(`/projects/${task.projectId}/${target}`);
    }
    ...
    const loadTask = useCallback(async () => {
        ...
        try {
            ...
            const [userList, projectData] = await Promise.all([
                fetchProjectUsers(data.projectId),
                fetchProject(data.projectId),
            ]);
            setUsers(userList);
            setEnableCost(projectData.enableCost ?? false);
        } catch {
        ...
        <div>
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div className="flex gap-2">
                    <Button variant="flat" onPress={backToProject}>
            ...
            <Card className="mb-6">
                ...
                <CardBody className="gap-4">
                    ...
                    <div className={`grid grid-cols-1 sm:${enableCost ? "grid-cols-3" : "grid-cols-1"} gap-4`}>
                        <Input
                            type="text"
                            inputMode="decimal"
                            label="HH by Task"
                            placeholder="0"
                            value={formatNumber(hhTask, 2)}
                            onValueChange={(val) => setHhTask(parseNumber(val))}
                        />
                        {enableCost && (
                            <>
                                <Input
                                    ...
                                    value={formatNumber(costTask)}
                                    onValueChange={(val) => setCostTask(parseNumber(val))}
                                />
