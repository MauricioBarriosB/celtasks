...
const PRIORITY_COLOR_MAP: Record<TaskPriority, "default" | "primary" | "warning" | "danger"> = {
    low: "default",
    medium: "primary",
    high: "warning",
    urgent: "danger",
...
export default function TaskCard({ task, canEdit, onDragStart }: Readonly<TaskCardProps>) {
    ...
    return (
        <div
            ...
            onKeyDown={(e) => { if (canEdit && e.key === "Enter") navigate(`/tasks/${task.id}`); }}
            className={canEdit ? "cursor-grab active:cursor-grabbing" : "cursor-default opacity-80"}
        >
            <Card
                isPressable={canEdit}
                onPress={() => canEdit && navigate(`/tasks/${task.id}`)}
                className={`w-full shadow-xl hover:shadow-md transition-shadow ${!canEdit ? "pointer-events-auto" : ""}`}
            >
